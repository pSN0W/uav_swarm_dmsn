...
class MISN:
    ...
    def get_sink_node_in_range(self, uav_projection: list[float, float],
                               max_dist: float) -> list[SinkNode]:
        ...
        for node in self.sink_nodes:
            ...
            if self.distance(sink_node_loc, uav_projection
                             ) <= max_dist and not node.mark_for_transference:
    ...
    def distance(point1: list[float, float], point2: list[float,
                                                          float]) -> float:
